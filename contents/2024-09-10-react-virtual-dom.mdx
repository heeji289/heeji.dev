...
이번에 리액트의 핵심 기능들을 직접 구현해보며 그 내부 동작을 깊이 이해하고자 프로젝트를 진행했습니다. 이 글에서는 JSX와 가상 DOM에 대해 구현 과정을 공유하려고 한다.
...
리액트 만들기 프로젝트에서의 첫 번째 과제는 JSX를 JavaScript로 변환하는 과정이었습니다. 처음에는 JSX 문법을 변환하는 파서를 직접 만들어 더 깊이 들어갈까 고민했습니다. 하지만 이게 내 주 목적에 맞나? 했을 때, 아니라는 결론이 나왔고 도구의 도움을 받기로 했습니다.

저는 프로젝트 세팅을 할 때, vite를 주로 사용합니다. vite는 번들러, 트랜스파일러, 개발 서버 등을 포함한 종합적인 프론트엔드 개발 환경 구축 도구입니다. https://ko.vitejs.dev/guide/ 참고. webpack, babel로 구축해본 적도 있는데 (이 글 참고) 이것저것 해주어야 할 것들이 많은데 vite는 명령어 한 줄이면 정말 간단하게 구축해주기 때문에 애용합니다.

```
💡 용어 설명
- 번들러: 여러 개의 파일을 하나의 파일로 묶어주는 도구입니다.
- 트랜스파일러: 한 프로그래
```
...
vite.config에서 esbuild 옵션을 지정해줄 수 있습니다. esbuild란?
...
export default defineConfig({
  esbuild: {
    ...
    jsxInject: `import { createElement } from '@/src/jsx-transpiler'`,
  },
});
```
...
![image.png](https://prod-files-secure.s3.us-west-2.amazonaws.com/44096953-0104-48ab-86b2-0f4d291e53be/63c3f798-a0a4-4b2b-b346-f668fd7ed96a/image.png)
